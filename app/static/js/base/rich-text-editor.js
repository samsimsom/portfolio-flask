    ...
    (win[NS] = function ($source, o) {
      ...
      var $_is = $.is,
        ...
        tools = {
          ...
          a: function (e) {
            var a = text(selection_v(1, 1, 0)).split(/\s/)[0],
              b,
              c
      ...
      function selection_v(h, x, p) {
        if (h) {
          if (($s = $s_get())) {
            ...
            return is_undef(p) || p
              ? h
              : h[replace](pattern(P_SPLIT_REGXP, 'g'), '\n\n')[replace](
                  pattern(P_ANY_REGXP, 'g'),
                  ''
                )
          }
          return ''
      ...
      function selection_a(o) {
        try {
          ...
          if (
            j &&
            (o[indexOf]('//' + j) === 0 || o[indexOf]('://' + j) !== -1)
          ) {
            // full path prefixed by URL protocol or `//`
            i = 1
          }
      ...
      win[NS][instance][
        $source.id || $source.name || Object.keys(win[NS][instance])[length]
      ] = $
      ...
      for (i in tools) {
        $.t[i] = tools[i]
        tools[i] = btn(
          c_text[i][0] + (c_text[i][2] ? ' (' + c_text[i][2] + ')' : ''),
          c_class + '-t:' + i,
          c_text[i],
          tools[i]
        )
      ...
      $.f = function (s) {
        ...
        s = s[replace](pattern(P_SPLIT_REGXP, 'gi'), '</p><p$1>')
          [replace](pattern(BR_ANY_REGXP, 'gi'), BR)
          [replace](/\n/g, BR)
          [replace](pattern('(?:' + BR_ANY_REGXP + '){3,}', 'gi'), BR + BR)
          [replace](
            pattern(
              '^(?:' + BR_ANY_REGXP + ')+|(?:' + BR_ANY_REGXP + ')+$',
              'gi'
            ),
            ''
          )
          [replace](pattern('(?:' + BR_ANY_REGXP + '){2,}', 'gi'), '</p><p>')
          [replace](pattern(P_EMPTY_REGXP, 'gi'), '')
        s =
          s && !pattern('^' + P_CONTAIN_REGXP + '$', 'gi')[test](s)
            ? '<p>' + s + '</p>'
            : s
      ...
      function write() {
        $view[innerHTML] = $.f($source[value])
          [replace](pattern(P_SPLIT_REGXP, 'gi'), BR + BR)
          [replace](pattern(P_ANY_REGXP, 'gi'), '')
      }
      write()
      function copy() {
      ...
      function fn_prompt_keydown(e) {
        ...
        if (!ctrl && !shift && (key === 'enter' || k === 13)) {
          ...
          (key === 'escape' ||
            k === 27 ||
            (!t[value][length] && (key === 'backspace' || k === 8)))
        ) {
          $.d.x(1, 1)
      ...
      $.set = function (v, i) {
        v = $.f(v)
        if ($_is[view] || i === 1) {
          v = v[replace](pattern(P_EDGE_REGXP, 'gi'), '')[replace](
            pattern(P_SPLIT_REGXP, 'gi'),
            BR + BR
          )
        }
        if (is_undef(i)) {
